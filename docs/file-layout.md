...
bin/omarchy-*                  ──►  omarchy             /usr/bin/omarchy-*
                                                        (and symlinks in /usr/share/de-omarchy/bin/)
bin/omarchy-debug,
...
install/**                     ──►  omarchy             /usr/share/de-omarchy/install/
migrations/**                  ──►  omarchy             /usr/share/de-omarchy/migrations/
themes/**                      ──►  omarchy             /usr/share/de-omarchy/themes/
shell/**                       ──►  omarchy             /usr/share/de-omarchy/shell/
version                        ──►  omarchy             /usr/share/de-omarchy/version
                                                        + /etc/skel/.local/state/omarchy/migrations/*

config/**                      ──►  omarchy-settings    /etc/skel/.config/**         (seeds new users)
                                                        /usr/share/de-omarchy/config/** (resync source)
...
applications/*.desktop         ──►  omarchy-settings    /etc/skel/.local/share/applications/
                                                        /usr/share/de-omarchy/applications/
default/applications/battlenet.desktop
                                ──►  omarchy-settings    /usr/share/de-omarchy/default/applications/
...
etc/**                         ──►  omarchy-settings    /etc/**           (drop-ins we own outright)
  ...
  └─ security/faillock.conf, nsswitch.conf,
     cups/cups-browsed.conf, plymouth/plymouthd.conf    /usr/share/de-omarchy/etc-overrides/
                                                          → /etc/* (post_install cp -f, see below)

default/limine/limine.conf     ──►  omarchy-settings    /usr/share/de-omarchy/default/limine/limine.conf
default/limine/default.conf    ──►  omarchy-settings    /usr/share/de-omarchy/default/limine/default.conf
...
default/snapper/root           ──►  omarchy-settings    /etc/snapper/config-templates/omarchy
                                                        (+ /usr/share/de-omarchy/default/snapper/root)

default/**                     ──►  omarchy-settings    /usr/share/de-omarchy/default/
  ├─ bash/env-bootstrap                                 /usr/share/de-omarchy/default/bash/env-bootstrap
  │                                                       (sourced by every shell/session entry point; see "Env bootstrap")
  ├─ bashrc                                             /usr/share/de-omarchy/etc-overrides/dot.bashrc
...
logo.{txt,svg}, icon.{txt,png}  ──► omarchy-settings    /usr/share/de-omarchy/  (resync source)
...
`/usr/share/de-omarchy/etc-overrides/` and the `omarchy-settings` `post_install`
...
- Sources `/etc/omarchy.conf` (written by `omarchy-dev-link`, reset to the
  ...
  `OMARCHY_PATH=/usr/share/de-omarchy` so a stale inherited value can't survive
  an `omarchy-dev-unlink`.
- Prepends `$OMARCHY_PATH/bin` to `PATH` **only when** `OMARCHY_PATH` is
  not `/usr/share/de-omarchy`. On a production install the binaries are
...
/usr/share/de-omarchy/default/bash/envs           (SSH / non-login bash)
```
