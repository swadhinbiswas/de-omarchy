-- ============================================================================
--  de-omarchy curated bindings
--
--  ONLY Omarchy default bindings that do NOT collide with the host (Moonrice)
--  keymap are bound here. Conflicting keys stay owned by the host config —
--  see audit/MAPPING.md for the full conflict table and
--  keybindings/KEYMAP.md for the resulting combined map.
--
--  Skipped on purpose (host already binds them):
--    SUPER+SPACE, SUPER+SHIFT+SPACE, SUPER+W, SUPER+T, SUPER+C, SUPER+V,
--    SUPER+X, SUPER+L, SUPER+ESCAPE, CTRL+ALT+DELETE, SUPER+F, SUPER+SHIFT+F,
--    SUPER+SHIFT+arrows, workspace digits, all XF86 media/brightness keys,
--    SUPER+CTRL+S, SUPER+CTRL+R, SUPER+SHIFT+COMMA
-- ============================================================================
...
-- Resize with minus/equals family (host uses arrows for focus/move instead).
...
---- Menus & utilities ----
----------------------------
-- Omarchy menu lives on SUPER+SHIFT+M here; upstream's SUPER+SPACE is taken by
-- the host config (toggle floating). Apps menu stays on SUPER+ALT+SPACE.
...
-- Notifications (SUPER+SHIFT+comma is host-owned: playerctl previous).
