...
**For end-user customization tasks, NEVER modify anything in `/usr/share/de-omarchy/`** - but READING is safe and encouraged.

This directory is owned by the omarchy package. Any local changes will be
overwritten on the next `omarchy update`.

```
/usr/share/de-omarchy/     # READ-ONLY - NEVER EDIT (reading is OK)
...
**Reading `/usr/share/de-omarchy/` is SAFE and useful** - do it freely to:
...
- Reference default hyprland settings: `cat /usr/share/de-omarchy/default/hypr/*`
...
2. **Is it a config edit?** Edit in `~/.config/`, never `/usr/share/de-omarchy/`
...
- Editing files in `/usr/share/de-omarchy/` (`bin/`, `config/`, `default/`, `shell/`, `themes/`, `migrations/`, etc.)
